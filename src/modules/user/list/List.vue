        ...
        <div class="card mb-4">
            <div class="grid">
                <div class="field col-12 md:col-6 lg:col-3">
                    ...
                    <Dropdown v-model="selectedRole" :options="roles" optionLabel="name" optionValue="code" placeholder="Select role" class="w-full" />
                </div>
            </div>
        </div>
        <div class="card">
            ...
            >
                ...
                <Column v-for="column in selectedColumns" :key="column.field" :field="column.field" :header="column.header" :sortable="column.sortable" :frozen="!isMobile && column.frozen" :alignFrozen="column.alignFrozen" :style="column.style">
                    ...
                    <template v-else-if="column.field === 'firstName'" #body="{ data }">
                        <Avatar :image="data.image" size="medium" shape="circle" class="mr-2" />
                        {{ data.firstName }}
                    </template>
                    <template v-else-if="column.field === 'actions'" #body="{ data }">
                        <div class="flex">
                            <router-link v-if="$can('edit', 'users')" :to="{ name: 'editUser', params: { id: data.id } }">
                                <Button type="button" icon="pi pi-pencil" class="mr-2 p-button-info p-button-rounded p-button-outlined custom-sm-button" />
                            </router-link>
                            <Button v-if="$can('delete', 'users')" type="button" icon="pi pi-trash" class="p-button-danger p-button-rounded p-button-outlined custom-sm-button" @click="showConfirmDialog(data.id)" />
                        </div>
                    </template>
                </Column>
            </DataTable>

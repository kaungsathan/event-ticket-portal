      ...
      >
        <template #header>
          <div class="flex flex-wrap justify-end gap-2">
            ...
            <!-- <router-link v-if="$can('manage', 'settings')" :to="{ name: 'newPayment' }">
        ...
        <Column v-for="column in selectedColumns" :key="column.field" :field="column.field" :header="column.header" :sortable="column.sortable" :frozen="!isMobile && column.frozen" :alignFrozen="column.alignFrozen" :style="column.style">
          <template v-if="column.field === 'actions'" #body="{ data }">
            <div class="flex gap-2">
              <router-link v-if="$can('manage', 'settings')" :to="{ name: 'editPayment', params: { id: data.id } }">
...
const { dt, lazyParams, totalRecords, records, isLoading, search, actionItems, actionMenu, onPage, onSort, toggleColumnMenu, selectedColumns, columns, columnMenu } = useList()
